...
public class ResourceLoader {
	...
	private static void loadDefault(String folder, List<TextureNamed> textures, int width, int height) {
		Path directory;
		try {
			URL url = ResourceLoader.class.getResource("/assets/guiadditions/textures/gui/" + folder + "/");
			if(url == null) {
				GuiAdditions.LOGGER.error("Failed to load default textures!");
				return;
			}
        ...
        try (DirectoryStream<Path> paths = Files.newDirectoryStream(directory)) {
			...
			GuiAdditions.LOGGER.error("Failed to load default textures! (" + folder + ")");
		}
	}
